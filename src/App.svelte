<script>
  ...
  function orientSkyToStar(starMesh) {
    ...
    const quat = new THREE.Quaternion().setFromUnitVectors(starDir, targetDir);
  ...
  onMount(async () => {
    ...
    constellations.forEach((constellation, index) => {
      const color =
        constellationColors[index % constellationColors.length] || 0xffffff;
      const group = new THREE.Group();
...
<style>
  .title-block {
    ...
    bottom: 20px;
    right: 20px;
    max-width: 30%; /* massimo 30% della larghezza viewport */
    background-color: rgba(0, 0, 0, 0.6); /* sfondo leggermente trasparente */
